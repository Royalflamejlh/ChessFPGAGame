...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	wdata[1] = val;
	int code = 0;
	code = alt_avalon_spi_command(SPI_0_BASE, 0, 2, wdata, 0, 0, 0);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	code = alt_avalon_spi_command(SPI_0_BASE, 0, nbytes + 1, wdata, 0, 0, 0);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	int code = 0;
	alt_u8 data[1];
	code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, 1, data, 0);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//return (data + nbytes);
	int code = 0;
	code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, nbytes, data, 0);
